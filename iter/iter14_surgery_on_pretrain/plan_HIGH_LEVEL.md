...
| 2️⃣ | `vjepa_2_1_pretrain_long` | Meta V-JEPA 2.1 ViT-G | continual SSL **10 ep** (compute-matched control, NO factor patching) | 🆕 TO RUN |
...
Meta V-JEPA 2.1 ──▶ pretrain (5 ep) ─────────────▶ surgery_3stage_DI (5 ep)
                       ...
                       │                              ┗━▶ surgery_noDI    (5 ep)
                       │
                       └──▶ pretrain_long (10 ep)  ←─── ❎ NO factor patching (control)
```
...
| **Δ3** | `surgical (5+5)` vs `pretrain_long (10)` | **factor patching is CAUSAL — not just extra steps** ⭐ | 🆕 |
...
| File | Change | LoC est |
|---|---|---|
| `configs/train/probe_pretrain_long.yaml` (NEW) | extends `probe_pretrain.yaml`; overrides `max_epochs.full: 10` | ~12 |
...
| `scripts/run_probe_train.sh` | add `pretrain_long` subcommand; thread `--init-from-ckpt` through `surgery_*` subcommands | ~30 |
| `scripts/run_probe_eval.sh` | add `vjepa_2_1_pretrain_long` to default `ENCODERS`; new `encoder_ckpt_for` + `encoder_predictor_ckpt_for` cases | ~15 |
...
| `pretrain_long` (10 ep) | ~20 h (linear 2× of pretrain) | ~$16 |
...
2. 🥈 **`pretrain_long (10 ep)`** in parallel/next — only needed once we have any signal on Δ2.
