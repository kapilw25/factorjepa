...
## 🟢 Status (2026-04-19): SANITY green + probe infra wired, 1K POC Step A running

- 🟢 **m10** (Grounded-SAM, Step A): **6.13 s/clip on 96 GB** (100-dense baseline). **Step A on 1K val_1k is RUNNING now** after `poc_simplified.n_clips: 100` fallback removal (#62 — m10 now derives clip_limit from `--subset` / `--local-data`, no yaml shortcut). Log shows `Clip limit: 1000` ✓ at line 20; observed ~2.65-5 s/clip on val_1k (dense clips slower), ETA ~45-55 min wall.
...
- 🟢 **m09c SANITY** (Step D.1): **ALL 3 STAGES PASS** — 0.4870 / 0.4901 / **0.4806** (2026-04-17 first Stage 3), re-confirmed 2026-04-19 post-probe-wiring at 0.4870 / 0.4898 / **0.4803**. 96 GB migration closed v7's 24 GB OOM for free — no v8 teacher-offload code needed (#58 resolved).
- 🆕 **Mid-training probe infrastructure landed** (2026-04-19, #63): at stage boundaries + `every_n_steps` cadence (`n_points: 30` default), m09c now fires `run_probe_eval` (Prec@K / mAP@K / Cycle@K with BCa 95 % CI on val_1k, bit-identical to m06 decision-gate math) AND `run_probe_val_loss` (JEPA val-loss on same probe clips). 4 plots per surgery run: `m09_train_loss.png`, `m09_val_loss.png` (33 pts), `probe_trajectory.png` (33 pts, 3 metrics + CI bands), plus BWT scalar in `training_summary.json.probe_trajectory_stats`. Overhead ~7 % on 2.7 h POC, scale-invariant (probe N fixed at 1000). All yaml keys fail-loud (`.get(default)` purged, CLAUDE.md-compliant).
- 🐛 **m09c POC on 100-dense** (Step D.2 first + second runs, 2026-04-17): flushed out 2 training-config bugs that only surface at POC scale — **#60 `max_epochs.poc: 1`** (3 optimizer steps silent near-no-op) and **#61 `warmup_steps: 200 > stage_steps: 99`** (LR never reached target, loss 0.50→0.476 warmup-truncated). Both fixed: max_epochs.poc→100, warmup replaced by `warmup_pct: 0.20` auto-scaling. 100-dense tier retired (3200 visits/clip overfitting = unpublishable).
- 🎯 **1K val_1k POC pipeline (in flight)**: ~10 h end-to-end on 96 GB — m10 (~50 min, RUNNING) + m11 (~8 min) + m05 frozen (~70 min) + m09c Surgery (~2.7 h with `max_epochs.poc: 20` + 33-point probe trajectory) + m05 surgical (~70 min) + m06 (~5 min) + ExPLoRA arm (~2.75 h). Delivers the real 3-arm Prec@K comparison with mid-training trajectory evidence.
- 📅 **Deadline fit (NeurIPS May 04)**: ~26 GPU-h remaining (canary/SANITY re-run cost ~1 h on 2026-04-19). 1K POC (10 h) + iteration buffer (4 h) + FULL 115K (~1.5 d = 36 h on 96GB batch×4) = **tight but feasible** if 1K Prec@K shows Surgery > Frozen cleanly. Otherwise 1K becomes the submission-ready tier.
