...
> 4-variant chain (`explora` + `surgery_2stage_noDI` + `surgery_2stage_loud_agent` + `surgery_3stage_DI`) on the ultra_hard 3,066-clip tier, paired BCa on `data/ultra_hard_3066_eval.json` (N=308, CI_half ≈ ±2.4 pp at p=0.5). Iter11 v3 hard-pivot replaces v2's 10K/eval_10k random tier (which saturated to Δ≈0).
...
for variant in surgery_2stage_noDI surgery_2stage_loud_agent surgery_3stage_DI; do
python3 -c "
import yaml, json
...
CACHE_POLICY_ALL=1 ./scripts/run_eval.sh configs/train/surgery_2stage_loud_agent.yaml # Instance/proc C
CACHE_POLICY_ALL=1 ./scripts/run_eval.sh configs/train/surgery_3stage_DI.yaml         # Instance/proc D
```
