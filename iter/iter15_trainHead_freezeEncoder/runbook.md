...
Status (2026-05-15): pre-flight + V0 + V2 + V3a + V3b + V4 + V5 done. Next = **V6**.
...
## Phase 5 V3 — m09c2 SANITY both variants (~10 min)
...
# Post-check (FLAT path — matches m09c2 writer + run_eval.sh:210-211 + 226-227)
ls -la outputs/sanity/m09c_surgery_3stage_DI_head/{student_encoder.pt,m09c_ckpt_best.pt,motion_aux_head.pt}
ls -la outputs/sanity/m09c_surgery_noDI_head/{student_encoder.pt,m09c_ckpt_best.pt,motion_aux_head.pt}
...
```bash
# Assert all 3 head-only variants kept encoder BIT-IDENTICAL to Meta init.
python -c "
import torch
...
for variant_path in [
    ...
    'outputs/sanity/m09c_surgery_3stage_DI_head/student_encoder.pt',
    'outputs/sanity/m09c_surgery_noDI_head/student_encoder.pt',
]:
    p = Path(variant_path)
    if not p.exists():
        print(f'SKIP {variant_path}')
        continue
    ...
    for k, v in sd.items():
        if k.startswith('blocks.') and k in meta and v.shape == meta[k].shape:
            ...
            if torch.allclose(v, meta[k]):
                n_match += 1
    print(f'{p.parts[-2]}/{p.name}: {n_match}/{n_total} bit-identical')
    assert n_match == n_total, f'FATAL: {variant_path} has {n_total - n_match} drifted blocks'
print('PASS: all 3 head-only variants bit-identical to Meta')"
```
...
```bash
bash scripts/run_eval.sh --sanity 2>&1 | tee logs/iter15_v6_full_eval_sanity_$(date +%Y%m%d_%H%M%S).log

# Pass: zero STAGE FAIL + probe_paired_delta.json has 7 delta_* keys
...
## HARD STOP — review V2-V6 before Phase 6

```bash
# Triage:
#   OOM at BS=1                  → 24 GB ceiling; check VRAM math
...
## Phase 6 — POC head-only training (~24 hr, ~$5)

```bash
# 3 independent cells — serial or 3 tmux sessions.
