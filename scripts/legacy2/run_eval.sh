...
#
# Reference: scripts/legacy2/run_paired_eval_10k.sh (frozen-share + per-variant chain pattern).
#
# USAGE:
#   ./scripts/legacy2/run_eval.sh <train-yaml1> [<train-yaml2> ...]
#   INCLUDE_BASELINES=1 ./scripts/legacy2/run_eval.sh <yamls...>     # bound the metric axis
#       (random/oracle/dinov2/clip/vjepa_shuffled — 5 extra m05b+m06 cycles, ~30 min)
#
# Example:
#   tmux new -s eval
#   INCLUDE_BASELINES=1 ./scripts/legacy2/run_eval.sh \
...
#       configs/legacy2/surgery_3stage_DI_multitask.yaml \
...
# Mirrors scripts/legacy2/run_paired_eval_10k.sh. Per-call-site × per-variant prompts:
...
for yaml in "$@"; do
    ...
    if [ ! -f "$ADAPTED_CKPT" ]; then
        echo "❌ ${VARIANT_TAG}: $ADAPTED_CKPT not found — run scripts/legacy2/run_train.sh first" >&2
        continue
    fi
