...
python -u src/utils/output_guard.py preflight_gpu evaluate 2>&1 | tee -a "$MASTER_LOG"
...
if [[ "$MODE" == "FULL" ]]; then
    SUBSET_N=$(python -u src/utils/config.py get-json data/subset_10k.json n)
    SUBSET_V=$(python -u src/utils/config.py get-json data/subset_10k.json num_videos)
    echo "Subset:         ${SUBSET_N} clips from ${SUBSET_V} videos" | tee -a "$MASTER_LOG"
fi
...
# ── Step 4: SKIPPED — m05c True Overlap (40h for marginal improvement over dim-split Overlap@K in m06)
# m06 already computes dim-split Overlap@K at zero extra cost. Same encoder ranking.
log "SKIPPED: m05c True Overlap (dim-split approximation in m06 is sufficient)"
...
python -u src/utils/output_guard.py verify_evaluate_final "$OUT_DIR" "$TAGS_FILE" 2>&1 | tee -a "$MASTER_LOG"
