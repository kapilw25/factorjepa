...
**Two views per clip.** For each clip *x*, we generate two spatiotemporal views: a *context view* $v_c \sim \mathcal{T}_c(x)$ and a *target view* $v_t \sim \mathcal{T}_t(x)$. The transforms $\mathcal{T}_c, \mathcal{T}_t$ are video-consistent augmentations applied with the same parameters across frames (e.g., one crop for all frames), so temporal structure is preserved.
...
**Student--teacher latent regression.** Let $S = f_\theta(v_c; M_c)$ denote student features computed from the context view, and let $T = f_{\bar{\theta}}(v_t; M_t)$ denote teacher features on masked target tokens (computed without gradients). The predictor produces $\hat{T} = g_\phi(S)$ aligned to the same masked token set. We minimize mean-squared error on the masked targets:

$$\mathcal{L}_{\text{JEPA}}(\theta, \phi) = \mathbb{E}\left[\|\hat{T} - \text{sg}(T)\|_2^2\right]$$
...
$$\bar{\theta} \leftarrow \tau\bar{\theta} + (1 - \tau)\theta, \quad \tau \in [0.99, 0.9999]$$
...
**Concrete implementable defaults.** A practical starting configuration is: clip length 10s; $T \in \{16, 32\}$ frames; input resolution 224 (or 256); target mask ratio 0.20 with 2--6 spatiotemporal blocks; mixed precision; EMA momentum $\tau$ warmed from ~0.996 to ~0.999; gradient clip 1.0; checkpoint every 2k--5k steps. These choices are directly implementable in standard training code (DDP + AMP).

### 10.5 Training loop (step-by-step)

**One training step.** Each step implements the following sequence:

- Sample a minibatch of clips from $\mathcal{D}$ (uniform over `video_id`).
- Decode/sample *T* frames per clip and apply the spatial resize/crop pipeline.
- Generate $v_c = \mathcal{T}_c(x)$ and $v_t = \mathcal{T}_t(x)$ using video-consistent transforms.
- Sample masks $(M_c, M_t)$ on the token grid.
- Student forward: $S = f_\theta(v_c; M_c)$.
- Teacher forward (no grad): $T = f_{\bar{\theta}}(v_t; M_t)$.
- Predictor: $\hat{T} = g_\phi(S)$; compute $\mathcal{L}_{\text{JEPA}}$ (plus $\mathcal{R}_{\text{stab}}$ if used).
- Backprop + optimizer step on $(\theta, \phi)$.
- EMA update of teacher parameters $\bar{\theta}$.
