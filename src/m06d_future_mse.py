...
USAGE (priority 1: forward on V-JEPA frozen only — DINOv2 path intentionally absent):
    python -u src/m06d_future_mse.py --FULL \\
        ...
        --output-root outputs/full/m06d_future_mse \\
        --cache-policy 1 2>&1 | tee logs/m06d_future_mse_forward_vjepa.log
...
# Variants understood by --paired_per_variant. Priority 1 only emits frozen;
# explora/surgery rows show up when iter11/12 ckpts get re-evaluated post-P1.
KNOWN_VARIANTS = (
    "vjepa_2_1_frozen",
    "vjepa_2_1_explora",
    "vjepa_2_1_surgical",
...
def run_forward_stage(args, wb) -> None:
    ...
    # Resume
    ckpt_file = out_dir / ".m06d_future_mse_ckpt.npz"
    ...
    if not target_keys:
        ...
        pbar = make_pbar(total=len(test_keys), desc="m06d_future_mse",
...
def run_paired_per_variant_stage(args, wb) -> None:
    ...
    Output: <output_root>/m06d_future_mse_per_variant.json
    ...
    save_json_checkpoint(out, args.output_root / "m06d_future_mse_per_variant.json")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Future-frame latent prediction MSE (m06d future_mse — V-JEPA-only diagnostic)")
    ...
    p.add_argument("--action-probe-root", type=Path, default=None,
                   help="m06d_action_probe output dir (provides action_labels.json test split)")
...
def main() -> None:
    ...
    wb = init_wandb(f"m06d_future_mse_{args.stage}", mode,
