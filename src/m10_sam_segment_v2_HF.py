"""Grounded-SAM video segmentation (HF Sam3TrackerVideoModel backend). GPU-only. [v2_HF]

Same Grounded-SAM pipeline as m10_sam_segment.py (Grounding DINO box detection → SAM 3 box-prompted
mask + propagation), but uses HuggingFace `transformers.Sam3TrackerVideoModel` instead of
`facebookresearch/sam3`'s `build_sam3_predictor`. The HF wrapper's `propagate_in_video_iterator`
implements `max_frame_num_to_track` + `start_frame_idx` correctly — no empty-tensor crashes
(errors #33/#34/#35 in raw sam3 pkg). Each anchor propagates only within its segment → ~10× speedup.

    python -u src/m10_sam_segment_v2_HF.py --SANITY --local-data data/val_1k_local --no-wandb 2>&1 | tee logs/m10_v2HF_sanity.log
    python -u src/m10_sam_segment_v2_HF.py --POC --subset data/sanity_100_dense.json --local-data data/val_1k_local --no-wandb 2>&1 | tee logs/m10_v2HF_dense100.log
    python -u src/m10_sam_segment_v2_HF.py --FULL --local-data data/full_local --no-wandb 2>&1 | tee logs/m10_v2HF_full.log
    python -u src/m10_sam_segment_v2_HF.py --POC --subset data/sanity_100_dense.json --local-data data/val_1k_local --no-wandb --probe-p3a 5 2>&1 | tee logs/m10_v2HF_probe5.log   # P-5a + P-3a A/B on 5 clips
    python -u src/m10_sam_segment_v2_HF.py --SANITY --plot                                                                                                                            # re-generate plots only (no GPU)
...
from transformers import (
    ...
    Sam3VideoModel,        # P-3a probe only: text-only video PCS
    Sam3VideoProcessor,    # P-3a probe only
)
...
# ── P-3a Probe: Sam3VideoModel text-only on 5 clips ──────────────────
# Goal: test if SAM3's unified PCS text encoder grounds our 17-cat Indian taxonomy
# directly on video (no DINO). If masks match P-5a quality → drop DINO for FULL
# (saves ~50% compute per clip). If Indian text grounding is weak (as it was in the
# raw sam3 pkg on dense100) → stick with P-5a.

def probe_p3a_text_only(clip_keys: list, frames_by_clip: dict, taxonomy: list,
                        out_dir: Path, model_id: str = "facebook/sam3"):
    """Run Sam3VideoModel (text-only PCS) on a handful of clips, save masks for A/B vs P-5a."""
    print(f"\n[P-3a probe] Sam3VideoModel text-only on {len(clip_keys)} clips ...")
    model = Sam3VideoModel.from_pretrained(model_id, dtype=torch.bfloat16).to("cuda").eval()
    proc = Sam3VideoProcessor.from_pretrained(model_id)
    out_dir.mkdir(parents=True, exist_ok=True)
    for clip_key in clip_keys:
        frames_np = frames_by_clip[clip_key]  # (T, H, W, 3) uint8
        session = proc.init_video_session(video=frames_np, inference_device="cuda", dtype=torch.bfloat16)
        # add_text_prompt: kwarg is `text=` (not `prompts=`) per transformers 5.5.4 API (#41)
        proc.add_text_prompt(inference_session=session, text=taxonomy)
        agent_union = np.zeros(frames_np.shape[:3], dtype=bool)  # (T, H, W)
        for out in model.propagate_in_video_iterator(inference_session=session, max_frame_num_to_track=frames_np.shape[0]):
            # postprocess_outputs returns {object_ids, scores, boxes, masks, prompt_to_obj_ids}
            # — key is "masks" (not "pred_masks"), tensor shape (N_objs, H, W)
            post = proc.postprocess_outputs(session, out)
            fidx = int(out.frame_idx)
            masks_t = post["masks"]  # tensor (N, H, W) bool
            if masks_t.numel() == 0:
                continue
            for i in range(masks_t.shape[0]):
                m_np = np.asarray(masks_t[i].detach().cpu(), dtype=bool)
                if m_np.shape != agent_union.shape[1:]:
                    m_np = np.array(Image.fromarray(m_np.astype(np.uint8) * 255).resize(
                        (agent_union.shape[2], agent_union.shape[1]), Image.NEAREST)) > 127
                agent_union[fidx] |= m_np
        safe = clip_key.replace("/", "__")
        np.savez_compressed(out_dir / f"{safe}__p3a.npz", agent_mask=agent_union, mid_frame=frames_np[len(frames_np)//2])
        print(f"  {clip_key}: P-3a agent_ratio = {agent_union.mean():.3%}")
    del model, proc; import gc; gc.collect()
...
def main():
    ...
    parser.add_argument("--probe-p3a", type=int, default=0, metavar="N",
                        help="After P-5a finishes, run Sam3VideoModel text-only probe on first N clips "
                             "(saves to masks_p3a/). 5 recommended; 0 disables.")
    ...
    t0 = time.time()
    probe_cache = {}  # for --probe-p3a: {clip_key: frames_np} on first N clips
    ...
    with tempfile.TemporaryDirectory(prefix="m10_") as tmp_dir:
        ...
        while n_processed < clip_limit:
            ...
            # Cache frames for P-3a probe (first N clips only)
            if args.probe_p3a > 0 and len(probe_cache) < args.probe_p3a:
                probe_cache[clip_key] = frames_np.copy()
    ...
    # P-3a probe: Sam3VideoModel text-only on cached clips, for A/B vs P-5a.
    if args.probe_p3a > 0 and probe_cache:
        probe_p3a_text_only(
            clip_keys=list(probe_cache.keys()),
            frames_by_clip=probe_cache,
            taxonomy=agent_taxonomy,
            out_dir=output_dir / "masks_p3a",
            model_id=sam_model_id,
        )

    # Force exit matches v1 behavior: HF Sam3 video sessions + DINO hold GPU buffers.
