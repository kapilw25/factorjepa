...
- Mandatory checklist for ANY GPU pipeline script: (1) tqdm progress bar (rule #13), (2) auto-resume from checkpoint (skip completed items on restart), (3) tee logging, (4) wandb integration, (5) windowed throughput reporting (rule #11), (6) **output-exists guard** — check if final output file exists at top of main() BEFORE loading model/streaming data; skip entirely if output exists and no partial checkpoint. Missing any = broken code that wastes GPU hours on re-runs. **ENFORCED: Claude MUST run `/preflight <file>` after ANY edit to `src/m*.py` that touches main() or the processing loop. No exceptions — this is not optional.**
