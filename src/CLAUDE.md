...
- **No DEFAULT, Silent error leads to research paper rejection** : No `.get(key, default)` on YAML — use `cfg[key]` so missing keys crash. FAIL LOUD. This is not a produiction project. 
...
- **CLI contract**: each .py (m04/m04d/m05/m05b/m05c/m06/m08/m08b/m09a/m09b/m09c/m10/m11) registers `--cache-policy {1,2}` via `utils.cache_policy.add_cache_policy_arg()`. `1=keep` (default, Enter) preserves all caches with a log line; `2=recompute` authorizes destruction through `guarded_delete(path, policy, label)`. No `.py` deletes anything without the flag reaching `2`.
- **Shell contract**: each `scripts/*.sh` runs one `read -p "... [1=keep / 2=recompute] (Enter=1): "` per unique py-call-site at the TOP, collects answers into shell vars, then fires every `python -u ...` with `--cache-policy "$VAR"`. Overnight/tmux runs gather all prompts up front, then proceed non-interactively. Env override: `CACHE_POLICY_ALL=1|2 ./script.sh` bypasses prompts.
- **Orthogonality with `output_guard`**: `output_guard.verify_or_skip()` prevents *redundant compute when outputs exist*; `cache_policy.guarded_delete()` prevents *destruction of cache without authorization*. Both layers required — deleting either would re-open the v1→v10 cycle.
