...
5) in each python script, keep DOCSTRING limited to terminal commands (covering --SANITY, --FULL, etc arguments) to be executed 
...
6) TEST [`py_compile` && `--help` && `ast` && `import`] using VIRTUAL ENVIRONMENT `source venv_*/bin/activate`
```
...
```
7) once all python files are built @src/ directory,
